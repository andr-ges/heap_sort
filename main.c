...
{	
	for(int sorted = 0; sorted<count ;sorted++)
	{
		for (int i = (count-sorted) / 2; i != 0; i--)
		{
				...
				(char*)ptr + 2 * i*size) < 0) 
			{
				...
				{	//swap 2i+1 and i
					...
					memmove(temp,(char*)ptr + (2 * i)+1*size,size);
					memmove((char*)ptr + (2 * i + 1)*size, (char*)ptr + i*size, size);
					memmove((char*)ptr + i*size, temp, size);
				}
				else	
				{//swap 2i and  i
					void *temp = malloc(size);
					memmove(temp, (char*)ptr + (2 * i) * size, size);
					memmove((char*)ptr + (2 * i)*size, (char*)ptr + i*size, size);
					memmove((char*)ptr + i*size, temp, size);
				}
			}
			if (cmp((char*)ptr + 2 * i*size,
		...
		{
			void *temp = malloc(size);
			memmove(temp, (char*)ptr + (sorted)* size, size);
			memmove((char*)ptr + (sorted)*size, (char*)ptr, size);
			memmove((char*)ptr, temp, size);
		}
	}
}
